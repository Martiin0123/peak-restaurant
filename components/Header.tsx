          ...
          >
            ...
            <nav className="hidden lg:flex items-center gap-1">
              {navItems.map((item) => (
                <a
                  ...
                  className="px-4 py-2 text-sm text-white/70 hover:text-white hover:bg-white/5 rounded-full transition-all"
                >
                  {item.label}
                </a>
              ))}
            ...
            <div className="hidden lg:flex items-center gap-6">
              ...
              <a
                ...
                className={`flex items-center gap-2 bg-taupe text-white px-5 py-2 rounded-full text-sm font-medium transition-transform hover:scale-105 ${isScrolled ? 'opacity-100' : 'opacity-100'}`}
              >
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            ...
            <nav className="relative z-10 flex flex-col items-center gap-8">
              {navItems.map((item, i) => (
                <motion.a
                  ...
                  className="text-5xl font-bold text-white/50 hover:text-white transition-colors"
                >
                  {item.label}
                </motion.a>
              ))}
            ...
            >
              <button onClick={() => setLanguage('de')} className={`text-xl ${language === 'de' ? 'text-taupe' : 'text-white/40'}`}>DE</button>
              <button onClick={() => setLanguage('en')} className={`text-xl ${language === 'en' ? 'text-taupe' : 'text-white/40'}`}>EN</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
